...
export default function MarketplaceScreen() {
  ...
  const list = useMemo(() => {
    ...
    if (amenities.length) out = out.filter((p) => amenities.every((a) => p.amenities.includes(a)));
    if (sort === "low") out = [...out].sort((a, b) => parsePrice(a.price) - parsePrice(b.price));
  ...
  return (
    <div style={{ background: colors.bg, color: colors.ink, position: "relative", zIndex: 0 }}>
      ...
      {detail ? (
        <PropertyDetail prop={detail} similar={similar} saved={saved} onSave={toggleSave} onBack={closeDetail} onOpen={openDetail} />
      ) : (
        <>
          {/* compact hero + search */}
          <section style={{ ...wrap, paddingTop: spacing.xl }}>
            ...
            >
              ...
              />
              <div style={{ position: "absolute", left: "clamp(22px, 3.5vw, 44px)", bottom: "clamp(20px, 3vw, 34px)", color: colors.white }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: fontSize.sm, color: "rgba(255,255,255,0.82)" }}>
                  <span>Home</span>
            ...
            >
              <div style={{ display: "flex", background: "#EFEFF2", borderRadius: 12, padding: 4, flexShrink: 0 }}>
              ...
              >
                ...
                <input
                  ...
                  style={{ border: "none", outline: "none", background: "none", width: "100%", fontSize: fontSize.base - 0.5, color: colors.ink }}
                />
              </label>
              <label
                style={{
          ...
          <section style={{ ...wrap, paddingTop: spacing.xxl + 6 }}>
            <div className="grid grid-cols-1 lg:grid-cols-[262px_1fr]" style={{ gap: spacing.xxl, alignItems: "start" }}>
              {/* filter sidebar */}
              <aside
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0 4px" }}>
                  ...
                  {activeCount > 0 && (
                    <button onClick={clearAll} style={{ fontSize: fontSize.sm, fontWeight: 600, color: colors.accent }}>
                      Clear all
                    </button>
                  )}
                </div>
                ...
                <FilterGroup title="Bedrooms">
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {bedOptions.map((b) => (
                      <SegPill key={b} label={`${b} BHK`} active={beds === b} onClick={() => setBeds(beds === b ? "" : b)} />
                    ))}
                  </div>
                </FilterGroup>
                ...
                <FilterGroup title="Bathrooms">
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {bathOptions.map((b) => (
                      <SegPill key={b} label={`${b}+`} active={baths === b} onClick={() => setBaths(baths === b ? "" : b)} />
                    ))}
                  </div>
                </FilterGroup>
                ...
                <FilterGroup title="Amenities" last>
                  {amenityOptions.map((a) => (
                    <CheckRow key={a} label={a} checked={amenities.includes(a)} onChange={() => toggle(amenities, setAmenities, a)} />
                  ))}
                </FilterGroup>
              </aside>
              ...
              <main>
                <div style={{ marginBottom: spacing.lg }}>
                  <h2 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(20px, 2.4vw, 28px)", fontWeight: 600 }}>
                    Properties for {purpose === "Rent" ? "rent" : "sale"} in Kochi
                  </h2>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: spacing.lg, gap: spacing.md, flexWrap: "wrap" }}>
                  <span className="lg:hidden" style={{ fontSize: fontSize.sm, fontWeight: 600, color: colors.muted }}>
                    {activeCount > 0 ? `${activeCount} filters active` : "All properties"}
                  </span>
                  <div style={{ display: "flex", alignItems: "center", gap: spacing.md, marginLeft: "auto" }}>
                    <div style={{ display: "flex", background: colors.card, border: `1px solid ${colors.line}`, borderRadius: 10, padding: 3 }}>
                ...
                {activeCount > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: spacing.lg }}>
                    ...
                    {amenities.map((a) => (
                      <Chip key={a} label={a} onRemove={() => toggle(amenities, setAmenities, a)} />
                    ))}
                    <button onClick={clearAll} style={{ fontSize: fontSize.xs + 0.5, fontWeight: 600, color: colors.accent, textDecoration: "underline" }}>
                      Clear all
                    </button>
                  </div>
                )}
                ...
                {list.length === 0 ? (
                  ...
                  >
                    ...
                    <h3 style={{ fontSize: fontSize.lg, marginBottom: 8 }}>No properties match your filters</h3>
                    <p style={{ color: colors.muted, marginBottom: spacing.lg }}>Try widening your budget or removing a filter.</p>
...
function PropertyRow({
  ...
  return (
    ...
    >
      <div style={{ position: "relative", minHeight: 200, background: colors.primarySoft }}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <div style={{ padding: spacing.lg + 2, display: "flex", flexDirection: "column", gap: spacing.sm + 1 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: spacing.md }}>
          <div>
            ...
            <h3 style={{ fontSize: fontSize.lg - 1, fontWeight: 700, margin: "8px 0 4px", lineHeight: 1.3 }}>{property.title}</h3>
            <p style={{ display: "flex", alignItems: "center", gap: 6, fontSize: fontSize.sm, color: colors.muted }}>
              <Icon name="location" size={15} /> {property.location}, {property.city}
            </p>
          </div>
          <button
            onClick={(e) => {
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px 16px", color: colors.ink2, fontSize: 13.5 }}>
        ...
        >
          ...
          <button
            style={{ background: colors.primary, color: colors.white, fontWeight: 600, fontSize: 13.5, padding: "10px 16px", borderRadius: 10 }}
          >
            View Detail
          </button>
        </div>
